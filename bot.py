...
async def on_member_remove(member):
    ...
    channel = [channel for channel in client.get_all_channels() if channel.name == 'ogólny'][0]
...
async def on_message(message):
    ...
    hi_words = ['yo', 'cześć', 'hej', 'hey', 'hi', 'dzień dobry', 'dzien dobry']
    if contains_words(hi_words, MSG):
        await message.channel.send('Yo')

    bye_words = ['bye', 'do widzenia', 'żegnam', 'goodbye', 'see you later', 'peace']
    ...
    music_words = ['recommend some music', 'recommend music', 'poleć muzykę', 'polec muzyke']
    ...
    count_members_words = ['count members', 'policz uzytkownikow', 'policz użytkowników']
    if contains_words(count_members_words, MSG):
        await message.channel.send("There's " + str(message.guild.member_count) + " people and bots on this channel")

    how_are_you_words = ['how are you', 'jak się masz', 'jak sie masz', 'co tam', 'how\'s it going']
